...
// Copyright (c) 2021-2024 The Truth Core developers
...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** TRU START */
    case OP_TRU_ASSET              : return "OP_TRU_ASSET";
    /** TRU END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_TRU_ASSET) { // OP_TRU_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == TRU_T) { // Check to see if TRU starts at 27 ( this->size() < 105)
                if ((*this)[28] == TRU_R)
                    if ((*this)[29] == TRU_U)
                        index = 30;
            } else {
                if ((*this)[28] == TRU_T) // Check to see if TRU starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == TRU_R)
                        if ((*this)[30] == TRU_U)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == TRU_TR) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == TRU_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == TRU_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == TRU_R) {
...
{
    // 1 OP_TRU_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_TRU_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_TRU_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_TRU_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_TRU_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
