...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for TRU
    ...
    if (p_rewardSnapshot.strDistributionAsset != "TRU") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is TRU with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using TRU differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "TRU") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Truth address (already validated during ownership snapshot creation)
